...
Import reports are generated by the CSV import pipeline. After processing a file the importer writes a timestamped Markdown report under the `reports/` directory (`reports/import-report-<timestamp>-<sessionId>.md`) and also updates the working copy `IMPORT_REPORT.md` at the repository root for quick access. The report includes a per-row anomaly table, import summary, and the importer policies applied during ingestion.
